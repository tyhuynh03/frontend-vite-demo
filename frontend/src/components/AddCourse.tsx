...
function AddCourse() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch('http://localhost:8000/api/auth/courses/', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setError(errorData.detail || 'Có lỗi xảy ra khi tạo khóa học');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-800">
      ...
      <div className="bg-slate-50 border-b">
        <div className="mx-auto max-w-7xl px-4 py-3">
          <nav className="text-sm">
            ...
            <span className="mx-2 text-slate-400">/</span>
            <span className="text-slate-600">Thêm khóa học</span>
          </nav>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900">Thêm khóa học mới</h1>
